...
load ("SOE_binaural_sound_8k.mat")
...
if addBinSig
    T = 8;
    sig = estimated_binaural(1:fs_resample*T)';
    n = noise_raw(1: fs_resample*T);
    SNR = snr(sig, n);
    k = 10^(SNR/20);
    binSig = sig / k;
end
...
for n = L+M:sigLenSample
    ...
    if addBinSig
        binSigSamples = flip(binSig(n-M+1:n, :));
        y_h1 = y + binSigSamples(:,1);
    end
